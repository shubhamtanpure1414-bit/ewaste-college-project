...
def get_db():
    ...
    # Connect with autocommit to prevent "InFailedSqlTransaction" errors
...
def admin_login():
    if request.method == "POST":
        ...
        # PROJECT OVERRIDE: Hardcoded credentials to ensure you can always access data
        if username == "admin" and password == "admin123":
            session["admin_id"] = 1
            session["admin_username"] = "admin"
            return redirect(url_for("admin_dashboard"))
        
        # Secondary check against database
        conn = get_db(); cur = conn.cursor()
...
def admin_dashboard():
    conn = get_db(); cur = conn.cursor()
    # Fetching all requests to show history
    cur.execute("""
        SELECT pr.*, u.name, u.email, e.item_name, e.category 
    ...
    # Sorting them for your template
...
# ... (Keep your other routes like add_waste, request_pickup, etc.) ...
